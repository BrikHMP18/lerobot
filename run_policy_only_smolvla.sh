...
DATASET_REPO_ID="${DATASET_REPO_ID:-${HF_USER}/eval_smolvla_dump-pocket-cleaning2}"
NUM_EPISODES="${NUM_EPISODES:-10}"
EPISODE_TIME_S="${EPISODE_TIME_S:-60}"
RESET_TIME_S="${RESET_TIME_S:-60}"
TASK="${TASK:-use the scoop-like end effector to gather sand from the dump pocket and deposit it into the primary crusher at the center}"
...
POLICY_PATH="${POLICY_PATH:-/home/leonardo/NONHUMAN/lerobot/outputs/hf_checkpoints/Autobrik__SO-ARM100-dump-pocket-cleaning2-smolvla}"
...
if ! RESOLVED_POLICY_PATH="$(resolve_policy_path "${POLICY_PATH}")"; then
  echo "Could not resolve valid SmolVLA pretrained_model from POLICY_PATH=${POLICY_PATH}" >&2
  exit 1
fi
