...
RUN_DIR="${RUN_DIR:-}"
if [[ -z "${RUN_DIR}" ]]; then
  RUN_DIR="$(ls -dt outputs/train/benchmark/*_benchmark_smolvla 2>/dev/null | head -n1 || true)"
fi
if [[ -z "${RUN_DIR}" || ! -d "${RUN_DIR}" ]]; then
  echo "Could not find RUN_DIR. Set RUN_DIR explicitly, e.g.:" >&2
  echo "  RUN_DIR=outputs/train/benchmark/20260215_123456_benchmark_smolvla ./run_upload_checkpoints_smolvla.sh" >&2
  exit 1
fi
...
MODEL_REPO="${MODEL_REPO:-Autobrik/SO-ARM100-dump-pocket-cleaning2-smolvla}"
DATASET_REPO_ID="${DATASET_REPO_ID:-Autobrik/SO-ARM100-dump-pocket-cleaning2}"
SEED="${SEED:-1000}"
POLICY_TYPE="${POLICY_TYPE:-smolvla}"
PRETRAINED_BASE="${PRETRAINED_BASE:-lerobot/smolvla_base}"
MODEL_CARD_PATH="${MODEL_CARD_PATH:-/tmp/README_SMOLVLA_MODEL.md}"
...
# SmolVLA finetuned on SO-ARM100 dump-pocket-cleaning

This is a **SmolVLA** policy finetuned with LeRobot.
