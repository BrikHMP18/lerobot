...
CONDA_ENV="${CONDA_ENV:-lerobot-pi}"
...
DATASET_REPO_ID="${DATASET_REPO_ID:-${HF_USER}/eval_pi05_dump-pocket-cleaning2}"
...
POLICY_PATH="${POLICY_PATH:-/home/leonardo/NONHUMAN/lerobot/outputs/hf_checkpoints/Autobrik__SO-ARM100-dump-pocket-cleaning2-pi05}"
...
if ! RESOLVED_POLICY_PATH="$(resolve_policy_path "${POLICY_PATH}")"; then
  echo "Could not resolve valid pi05 pretrained_model from POLICY_PATH=${POLICY_PATH}" >&2
  exit 1
fi
