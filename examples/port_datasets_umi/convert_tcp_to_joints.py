...
class PiperInverseKinematics:
    ...
    def _fk_format_to_tcp(self, fk_pose):
        """Convert FK format to LeRobot TCP format.

        Args:
            fk_pose: [x, y, z, roll, pitch, yaw] (mm, Euler degrees)
        ...
        roll_rad = fk_pose[3] * math.pi / 180
        pitch_rad = fk_pose[4] * math.pi / 180
        yaw_rad = fk_pose[5] * math.pi / 180

        rot = Rotation.from_euler("xyz", [roll_rad, pitch_rad, yaw_rad], degrees=False)
        axis_angle = rot.as_rotvec()

        return [x_m, y_m, z_m, axis_angle[0], axis_angle[1], axis_angle[2]]
