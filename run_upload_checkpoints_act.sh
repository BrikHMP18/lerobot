...
RUN_DIR="${RUN_DIR:-}"
if [[ -z "${RUN_DIR}" ]]; then
  RUN_DIR="$(ls -dt outputs/train/benchmark/*_benchmark_act 2>/dev/null | head -n1 || true)"
fi
if [[ -z "${RUN_DIR}" || ! -d "${RUN_DIR}" ]]; then
  echo "Could not find RUN_DIR. Set RUN_DIR explicitly, e.g.:" >&2
  echo "  RUN_DIR=outputs/train/benchmark/20260215_071412_benchmark_act ./run_upload_checkpoints_act.sh" >&2
  exit 1
fi
...
MODEL_REPO="${MODEL_REPO:-Autobrik/SO-ARM100-dump-pocket-cleaning2-act}"
DATASET_REPO_ID="${DATASET_REPO_ID:-Autobrik/SO-ARM100-dump-pocket-cleaning2}"
SEED="${SEED:-1000}"
POLICY_TYPE="${POLICY_TYPE:-act}"
MODEL_CARD_PATH="${MODEL_CARD_PATH:-/tmp/README_ACT_MODEL.md}"
...
if best_val_path.exists():
    try:
        ...
        best_step = payload.get("step", payload.get("best_val_step", "N/A"))
        best_loss = payload.get("loss", payload.get("best_val_loss", "N/A"))
    except Exception:
        pass
...
# ACT finetuned on SO-ARM100 dump-pocket-cleaning

This is an **ACT** policy finetuned with LeRobot.
